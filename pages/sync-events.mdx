...
```
curl --request GET \
     --url https://sync.morgen.so/v1/events/list?calendarIds=<CALENDAR_IDS>&start=<START_DATETIME>&end=<END_DATETIME> \
...
curl --request POST \
     ...
     --header 'accept: application/json' \
     --header 'Authorization: Bearer <JWT>'
     --data '{"calendarId": <CALENDAR_ID>, ...eventfields}'
```
...
```
curl --request POST \
     --url https://sync.morgen.so/v1/events/update \
     --header 'accept: application/json' \
     --header 'Authorization: Bearer <JWT>'
     --data '{
...
curl --request POST \
     ...
     --header 'accept: application/json' \
     --header 'Authorization: Bearer <JWT>'
     --data '{"calendarId": <CALENDAR_ID>, "morgen.so:requestVirtualRoom": "default", ...eventfields}'
```
...
curl --request POST \
     ...
     --header 'accept: application/json' \
     --header 'Authorization: Bearer <JWT>'
      --data '{"id": <EVENT_ID>, "comment": <COMMENT>, "notifyOrganizer": <true|false>}'
```
