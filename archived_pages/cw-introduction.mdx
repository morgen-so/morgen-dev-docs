...
<Steps>
    ### Sign in to the Morgen platform:

    Create a new account or sign in to [Morgen platform](https://platform.morgen.so/dashboard).
...
async function getOpenAIResponse(prompt) {
  const resp = fetch("https://api.openai.com/v1/chat/completions", {
    method: "POST",
    headers: {
...
async function run(trigger) {
  ...
  const resp = await fetch("https://api.slack.com/api/chat.postMessage", {
    method: "POST",
    headers: {
